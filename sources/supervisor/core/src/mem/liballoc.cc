...
/*******************************************************************************
 * This is liballoc 1.1 taken from https://github.com/blanham/liballoc and originally by Durand Miller. I have modified
 *it a
...
#define ALIGN_INFO                                                                                                     \
    sizeof(ALIGN_TYPE)                                                                                                 \
        * 16 ///< Alignment information is stored right before the pointer. This is the number of bytes of
...
extern "C" auto preinit_liballoc(
    Conurbation::UEFI::handle_t ImageHandle, Conurbation::UEFI::efi_system_table_t* SystemTable) -> void
{
    uint64_t initial_heap_addr;
    // typedef status_t(efiabi efi_allocate_pages_f)(allocate_type_t type, memory_type_t memoryType, uintptr_t pages,
    // uint64_t*
    // memory);
    ...
    Conurbation::UEFI::status_t st
        = SystemTable->BootServices->AllocatePages(Conurbation::UEFI::allocate_type_t::AllocateAnyPages,
            Conurbation::UEFI::memory_type_t::OsData, _Conurbation_Preinit_Heap_Pages, &initial_heap_addr);

    if (st != Conurbation::UEFI::status_t::Success) {
        SystemTable->ConOut->OutputString(SystemTable->ConOut, u"Unable to intialize kernel heap from UEFI memory\r\n");
        SystemTable->BootServices->Exit(ImageHandle, Conurbation::UEFI::status_t::Aborted, 0, nullptr);
    }
