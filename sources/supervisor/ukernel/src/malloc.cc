...
#define MIN_FREE_CHUNK_SIZE (sizeof(struct free_header) + sizeof(struct footer))

#define MIN_INUSE_CHUNK_SIZE (sizeof(struct inuse_header) + sizeof(struct footer))
...
static const size_t bin_sizes[]
    = { 8,         16,        24,        32,        40,        48,         56,         64,
        ...
        576,       640,       768,       1024,      2048,      4096,       0x2000,     0x4000,
        0x8000,    0x10000,   0x20000,   0x40000,   0x80000,   0x100000,   0x200000,   0x400000,
        0x800000,  0x1000000, 0x2000000, 0x4000000, 0x8000000, 0x10000000, 0x20000000, 0x40000000,
        0x80000000 };
 ...
 */
inline static struct free_header* find_chunk(struct free_header* bin, size_t size) {
 ...
 * @return a pointer to the chunk's free header
 */
inline static struct free_header* find_upper_chunk(struct free_header* bin, size_t size) {
    struct free_header* chunk = bin;
...
static void add_free_chunk(void* memory, size_t size) {
    ...
    header = (struct free_header*)memory;
...
void* malloc(size_t size) {
    ...
    if (size < chunk->size && size <= context->last_chunk_size && size <= MAX_SMALL_REQUEST) {
...
void free(void* memory) {
    ...
    assert((char*)header + header->size <= (char*)context || (char*)header >= (char*)(context + 1));
...
void* realloc(void* memory, size_t size) {
    ...
    if (next_header->status == FREE_STATUS && next_header->size + (size_t)header->size < size) {
...
void set_malloc_context(void* new_context) {
    ...
    context = (struct memory_context*)new_context;
}
